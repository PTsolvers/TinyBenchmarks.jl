...
function main(::Type{DAT}; device) where DAT
    ...
    # warmup
    compute!(comp_flux!, comp_mbal!, A, qx, qy, C, dt, _dx, _dy, nt, ranges, comm_cart)
    compute!(comp_flux!, comp_mbal!, A, qx, qy, C, dt, _dx, _dy, nt, comm_cart)
    TinyKernels.device_synchronize(device)
    # split
    t_nt = compute!(comp_flux!, comp_mbal!, A, qx, qy, C, dt, _dx, _dy, nt, ranges, comm_cart)
    ...
    TinyKernels.device_synchronize(device)
    # no split
    t_nt = compute!(comp_flux!, comp_mbal!, A, qx, qy, C, dt, _dx, _dy, nt, comm_cart)
    t_eff = sizeof(eltype(A)) * nIO * length(A) * 1e-9 / minimum(t_nt)
